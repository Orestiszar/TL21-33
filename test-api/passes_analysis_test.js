...
module.exports = function(){
    describe('Passes analysis GET request test', () => {
        // after(function () {
        //     process.exit(0);
        // });
        var csv = fs.readFileSync('./testcases/passes_analysis_test.csv');
        csv = CsvToJson(csv);
    
        for(var test = 0; test < 10; ++test) {
            let temp = parseInt(csv[test].numberofpasses);
            
            let url = '/interoperability/api/PassesAnalysis/'+csv[test].op1+'/'+csv[test].op2+'/'+csv[test].datefrom+'/'+csv[test].dateto;
